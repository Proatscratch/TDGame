    ...
    def update(self, avoid, unit, dt):
        ...
        if (self.HIT == "GROUND"):
            ...
            if (self.position.y < 0 or has_collide):
                self.HIT = ""
        if (self.HIT == ""):
          #  print(self.renderObject.spritesheet.current_frame)
...
class Fighter: 
    ...
    def update(self, dt, enemies, avoid, projectile):
        ...
        if (self.animation == ATTACK and self.animation != IDLE) and self.frr and target:
            if len(self.frr[0]) == 3 and glm.distance(self.position, target.position) < self.frr[0][1] and self.spriteSheets[self.animation].current_frame in [pp[0] for pp in self.frr]:
                target.HP -= self.frr[0][2]
                self.frr = self.frr[1:]
            elif self.spriteSheets[self.animation].current_frame in [pp[0] for pp in self.frr]:
                #1/2at^2 + v0t = 0
                #(-v0 +/-v0)/a
                TIME = (- 2 * self.frr[0][1]) / self.frr[0][2]
                bvec = (self.target-self.position) / TIME
                bvec.y = self.frr[0][1]
                accel = glm.vec3(0, self.frr[0][2], 0)
                obj = QUAD.duplicate()
                obj.spritesheet = self.frr[0][6].duplicate( )
                rend.scene_objects.append(obj)
                projectile.append(Projectile(bvec, obj,accel, self.frr[0][3], self.frr[0][4], self.frr[0][5], glm.vec3(self.position), self.enemy))
                self.frr = self.frr[1:]
        ...
        if self.renderObject.vertices is QUAD.vertices:
            ...
            self.renderObject.rotation.y = glm.mix(self.renderObject.rotation.y, -math.atan2(self.target.z-self.position.z, self.target.x-self.position.x) - 3.14/2, dt * 1)
...
def FighterData(fighter, position, enemy):
    ...
    #print([[z, "range" in z] for I, z in enumerate(fighter["plots"])])
